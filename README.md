...
本项目实现了课程论文《基于 Transformer-LSTM 与方向感知保形预测的微电网功率预测及可调能力估计》中的完整实验代码。方法流水线包含三个阶段：**点预测**（LSTM/Transformer/PatchTST/ARIMA）→ **不确定性量化**（Split CP/DA-CP/加权 CP/归一化 CP）→ **可调能力估计**。

**核心创新**：提出方向感知保形预测（DA-CP），针对微电网运行中上调不足代价远大于下调过度的物理非对称性，在上调和下调方向分别采用 α/2 和 α 的差异化覆盖率目标，将上调越界率降低约 44%。
...
Remote Microgrid Dataset（chandar-lab, Mila），偏远风-柴-储微电网 2018 年全年 1 分钟分辨率数据，525,600 条。

## 运行

```bash
python -m src.data.preprocessing          # 数据清洗
python scripts/run_full_pipeline.py       # 全量实验
```
